  ...
  switch (type) {
    case 1:
      if (ele.src.includes('https://cn.gravatar.com/avatar')) {
        ele.src = ele.src.replace('https://cn.gravatar.com/avatar/', '/images/other/gravatar.svg')
      } else {
        ele.src = '/images/other/default-avatar.svg'
      }
      break
    case 2:
      ele.src = '/images/other/default-avatar.svg'
    ...
    case 3:
      ...
      ele.src = '/images/other/image-404.svg'
      break
    default:
      ele.src = '/images/other/image-404.svg'
  }
...
function nextBG () {
  ...
  $('.centerbg').css('background-image', setDoubleBG(bg[Math.abs(bgindex % bg.length)]))
}
...
/* 双层背景：同一 URL 铺两层，CSS 里前景 contain 完整显示、后景 cover + 模糊填充，
   解决竖图 cover 到宽屏头图只露中部横条（"大胸"）的问题 */
function setDoubleBG (url) {
  var u = 'url("' + url + '")'
  return u + ', ' + u
}
$(document).ready(function () {
  var bgindex = Math.floor(Math.random() * bg.length)
  $('.centerbg').css('background-image', setDoubleBG(bg[bgindex]))
...
var home = location.href,
  ...
  Siren = {
    ...
    VA: function () {
      if (typeof Valine !== 'undefined' && !valine) {
